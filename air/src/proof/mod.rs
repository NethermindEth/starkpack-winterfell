...
///
/// A STARKPack proof contains information proving that the computations were executed correctly. A proof
/// also contains basic metadata for the computations, but neither the definition of the computations
/// itself, nor public inputs consumed by the computations are contained in a proof.
///
...
impl StarkProof {
    /// Returns i-th STARK protocol parameters used to generate this proof.
    ...
    /// Returns a STARKPack proof read from the specified `source`.
    ///
    /// # Errors
    /// Returns an error of a valid STARKPack proof could not be read from the specified `source`.
